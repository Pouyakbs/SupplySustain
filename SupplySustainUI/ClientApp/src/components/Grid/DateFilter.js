            ...
            <div className='col-12'>
                <div className="position-relative filter-date-picker">
                    <DatePicker
                        ...
                        id={"firstDate"}
                        ref={dateRef1}
                        value={formik?.values[field]?.value1?getLangDate(i18n.language,formik?.values[field]?.value1):''}
                        calendar={renderCalendarSwitch(i18n.language)}
                        locale={renderCalendarLocaleSwitch(i18n.language)}
                        onChange={(val) => {
                            if(val){
                                formik.setFieldValue(`${field}.value1`,julianIntToDate(val.toJulianDay()))
                            }else{
                                formik.setFieldValue(`${field}.value1`,'')
                            }
                        }}
            ...
            <div className='col-12'>
                <div className="position-relative filter-date-picker">
                    <DatePicker
                        ...
                        id={"firstDate"}
                        ref={dateRef2}
                        value={formik?.values[field]?.value2?getLangDate(i18n.language,formik?.values[field]?.value2):''}
                        calendar={renderCalendarSwitch(i18n.language)}
                        locale={renderCalendarLocaleSwitch(i18n.language)}
                        onChange={(val) => {
                            if(val){
                                formik.setFieldValue(`${field}.value2`,julianIntToDate(val.toJulianDay()))
                            }else{
                                formik.setFieldValue(`${field}.value2`,'')
                            }
                        }}
                        disabled={!(formik?.values[field]?.value1||formik?.values[field]?.value2)}
                    />
